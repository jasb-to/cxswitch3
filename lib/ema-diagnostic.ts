...
export interface EmaDiagnostic {
  ...
  spreadAtr: number;
  spreadPrev: number;
  spreadContracting: boolean;
  spreadChangePct: number;
  ema5Slope: number;
  ema13Slope: number;
  crossNow: boolean;
  candlesFromCross: number | null;
...
export function get4HEmaDiagnostic(candles4h: Candle[]): EmaDiagnostic {
  ...
  if (closes.length < 20) {
    return {
      stage: "NEUTRAL", label: "NEUTRAL", direction: "NEUTRAL",
      ema5: 0, ema13: 0, ema5Prev: 0, ema13Prev: 0,
      spread: 0, spreadPct: 0, spreadAtr: 0, spreadPrev: 0,
      spreadContracting: false, spreadChangePct: 0, ema5Slope: 0, ema13Slope: 0,
      crossNow: false, candlesFromCross: null,
      closedCandleTimestamp: candles4h.at(-1)?.timestamp ?? 0,
    };
  }

  const e5 = ema(closes, 5), e13 = ema(closes, 13);
  const i = e5.length - 1, p = i - 1;
  ...
  const ema5Slope = ema5 - ema5Prev;
  const ema13Slope = ema13 - ema13Prev;
  const bullishCross = ema5Prev <= ema13Prev && ema5 > ema13;
  const bearishCross = ema5Prev >= ema13Prev && ema5 < ema13;

  // Level 2 is deliberately stricter than Level 1: the fast EMA must be moving
  // in the transition direction and the EMA gap must be contracting.
  ...
  return {
    stage, label, direction,
    ema5, ema13, ema5Prev, ema13Prev,
    spread,
    spreadPct: ema13 !== 0 ? (spread / ema13) * 100 : 0,
    spreadAtr: atrValue > 0 ? spread / atrValue : 0,
    spreadPrev,
    spreadContracting,
    spreadChangePct,
    ema5Slope,
    ema13Slope,
    crossNow: bullishCross || bearishCross,
    candlesFromCross: bullishCross || bearishCross ? 0 : null,
