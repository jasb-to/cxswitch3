...
// ============================================================
// v36.1 ALERTS — Updated for new signal format
// ============================================================

export async function sendAlert(signal: any): Promise<boolean> {
  // v36 signals use entryType instead of entryTier/scale
  ...
  const label = entryType === "EARLY" ? "EARLY ENTRY" : entryType === "RETEST" ? "RETEST ENTRY" : "BREAKOUT ENTRY";
  const size = entryType === "EARLY" ? "33% starter" : entryType === "RETEST" ? "50% add" : "FULL SIZE";
...
export async function alertStatus(signals: any[], prices: Record<string, number>): Promise<boolean> {
  ...
  const lines = signals.map(s => {
    ...
    const pnl = isFinite(rawPnl) ? rawPnl : 0;
    const sign = pnl >= 0 ? "+" : "";
    const entryType = s.entryType || "OPEN";
