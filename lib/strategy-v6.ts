...
/**
 * v16.4.0: SIGNAL LIFECYCLE ENGINE
 * 
 * Complete lifecycle state machine for signals.
 * Every signal exists in EXACTLY one lifecycle phase.
 * No implicit states. No UI-level state derivation.
 * 
 * States:
 *   NONE - signal does not exist
 *   BUILDING - setup forming, not yet executable
 *   ACTIVE_SNIPER - executable, ready for entry
 *   ACTIVE_CONFIRMED - trade active or confirmed
 *   EXPIRED - was active, timed out (TTL exceeded)
 *   INVALIDATED - was active, conditions degraded
 */
...
export type SignalLifecycle = {
  ...
  derivedState: DerivedSignal;  // Current derived signal (from deriveSignal)
  previousState: SignalLifecycleState;  // Previous lifecycle state (for transition detection)
  
  // Lifecycle metadata
  activatedAt?: number;  // Timestamp when entered ACTIVE_SNIPER or ACTIVE_CONFIRMED
  expiredAt?: number;  // Timestamp when transitioned to EXPIRED
  invalidatedAt?: number;  // Timestamp when transitioned to INVALIDATED
  invalidationReason?: string;  // Why was it invalidated
  
  // Visibility/rendering flags
  renderable: boolean;  // Can UI render this signal
  hasActiveTrade: boolean;  // Whether this signal currently has an active trade plan
  ttlMs: number;  // Time-to-live in milliseconds (0 if not time-bounded)
};

/**
 * v16.4.0: CANONICAL LIFECYCLE REDUCER
 * 
 * Single source of truth for all signal lifecycle transitions.
 * Input: previous lifecycle state, current derived signal, market data
 * Output: new canonical lifecycle state with full metadata
 * 
 * This function is the ONLY authority for:
 * - Activation (BUILDING → ACTIVE_SNIPER)
 * - Persistence (ACTIVE_SNIPER → ACTIVE_CONFIRMED)
 * - Downgrade (ACTIVE_CONFIRMED → BUILDING)
 * - Expiration (ACTIVE_* → EXPIRED)
 * - Invalidation (ACTIVE_* → INVALIDATED)
 */
export function deriveSignalLifecycle(
  symbol: string,
  previousLifecycle: SignalLifecycle | undefined,
  currentDerived: DerivedSignal,
  currentMarket: {
    ignition: number;
  ...
  const previousState = previousLifecycle?.lifecycleState || "NONE";
  const previousActivatedAt = previousLifecycle?.activatedAt;
  
  // STEP 1: Check expiration if currently active
  if ((previousState === "ACTIVE_SNIPER" || previousState === "ACTIVE_CONFIRMED") && previousActivatedAt) {
    const activeMs = nowMs - previousActivatedAt;
    const ttlMs = previousState === "ACTIVE_SNIPER" ? 15 * 60 * 1000 : 30 * 60 * 1000;  // 15m or 30m
    
    if (activeMs > ttlMs) {
      console.log(`[SIGNAL_EXPIRED] ${symbol}: ${previousState} exceeded TTL (${Math.round(activeMs / 1000)}s > ${Math.round(ttlMs / 1000)}s)`);
      return {
        symbol,
        lifecycleState: "EXPIRED",
        derivedState: currentDerived,
        previousState,
        expiredAt: nowMs,
        invalidationReason: `TTL exceeded (${previousState})`,
        renderable: false,
  ...
  // STEP 2: Check invalidation conditions (if currently active)
  if (previousState === "ACTIVE_SNIPER" || previousState === "ACTIVE_CONFIRMED") {
    const invalidationReason = checkInvalidationConditions(
      symbol,
      currentDerived,
      currentMarket,
      previousState
    ...
    if (invalidationReason) {
      console.log(`[SIGNAL_INVALIDATED] ${symbol}: ${previousState} → INVALIDATED (${invalidationReason})`);
      return {
        symbol,
  ...
  // STEP 3: Derive new lifecycle state based on current derived signal
  let newLifecycleState: SignalLifecycleState;
  let activatedAt: number | undefined = previousActivatedAt;
  
  switch (currentDerived.state) {
    case "NONE":
      newLifecycleState = "NONE";
      activatedAt = undefined;
      break;
      
    case "BUILDING":
      newLifecycleState = "BUILDING";
      if (previousState === "NONE") {
        console.log(`[SIGNAL_CREATED] ${symbol}: BUILDING (ignition=${currentMarket.ignition})`);
      }
      activatedAt = undefined;
      break;
      
    case "ACTIVE_SNIPER":
      if (previousState === "NONE" || previousState === "BUILDING") {
        console.log(`[SIGNAL_ACTIVATED] ${symbol}: ${previousState} → ACTIVE_SNIPER`);
        activatedAt = nowMs;
      }
      newLifecycleState = "ACTIVE_SNIPER";
      break;
      
    case "ACTIVE_CONFIRMED":
      if (previousState === "ACTIVE_SNIPER") {
        console.log(`[SIGNAL_UPGRADED] ${symbol}: ACTIVE_SNIPER → ACTIVE_CONFIRMED`);
        activatedAt = activatedAt || nowMs;  // Preserve or set activation time
      } else if (previousState !== "ACTIVE_CONFIRMED") {
        console.log(`[SIGNAL_ACTIVATED] ${symbol}: ${previousState} → ACTIVE_CONFIRMED`);
        activatedAt = nowMs;
      }
      newLifecycleState = "ACTIVE_CONFIRMED";
      break;
  }
  
  // STEP 4: Detect downgrade
  if (isDowngrade(previousState, newLifecycleState)) {
    console.log(`[SIGNAL_DOWNGRADED] ${symbol}: ${previousState} → ${newLifecycleState}`);
  }
  
  // STEP 5: Determine renderable flag
  const renderable = newLifecycleState === "BUILDING" || newLifecycleState === "ACTIVE_SNIPER" || newLifecycleState === "ACTIVE_CONFIRMED";
  
  // STEP 6: Determine if has active trade plan
  const hasActiveTrade = (newLifecycleState === "ACTIVE_SNIPER" || newLifecycleState === "ACTIVE_CONFIRMED") && !!currentDerived.targets;
  ...
  return {
    ...
    renderable,
    hasActiveTrade,
    ttlMs: newLifecycleState === "ACTIVE_SNIPER" ? 15 * 60 * 1000 : (newLifecycleState === "ACTIVE_CONFIRMED" ? 30 * 60 * 1000 : 0),
  };
}

/**
 * Check if current signal conditions warrant invalidation
 */
function checkInvalidationConditions(
  symbol: string,
  derived: DerivedSignal,
  market: any,
  previousState: string
): string | null {
  // Invalidation rule 1: Direction flip
  if (previousState !== "NONE" && previousState !== "BUILDING" && derived.direction === "NEUTRAL") {
    return "Direction became NEUTRAL";
  }
  
  // Invalidation rule 2: Ignition dropped below threshold
  if (previousState === "ACTIVE_SNIPER" && market.ignition < 60) {
    return `Ignition dropped below SNIPER threshold (${market.ignition}% < 60%)`;
  }
  if (previousState === "ACTIVE_CONFIRMED" && market.ignition < 75) {
    return `Ignition dropped below CONFIRMED threshold (${market.ignition}% < 75%)`;
  }
  
  // Invalidation rule 3: Missing trade plan for active state
  if ((previousState === "ACTIVE_SNIPER" || previousState === "ACTIVE_CONFIRMED") && !derived.targets) {
    return "Trade plan missing for active state";
  }
  
  // Invalidation rule 4: Market structure incompatible with previous state
  if (market.marketClass === "CHOP" || market.marketClass === "RANGE") {
    if (previousState === "ACTIVE_SNIPER" || previousState === "ACTIVE_CONFIRMED") {
      return `Market structure degraded to ${market.marketClass}`;
    }
  }
...
/**
 * Detect if this is a downgrade transition
 */
function isDowngrade(from: SignalLifecycleState, to: SignalLifecycleState): boolean {
  const stateRank = {
    "NONE": 0,
    "BUILDING": 1,
    "ACTIVE_SNIPER": 2,
  ...
  return (stateRank[from] || 0) > (stateRank[to] || 0);
}

/**
 * v16.4.0: INVARIANT CHECKER
 * 
 * Validates that signal states respect rendering invariants.
 * Detects impossible state combinations before they can reach the UI.
 * Returns list of violations (empty if valid).
 */
export function checkSignalInvariants(lifecycle: SignalLifecycle): string[] {
  const violations: string[] = [];
  const { symbol, lifecycleState, derivedState, hasActiveTrade, renderable } = lifecycle;
  
  // Invariant 1: ACTIVE_SNIPER must have ignition >= 60
  if (lifecycleState === "ACTIVE_SNIPER" && derivedState.confidence < 60) {
    violations.push(`[INV] ${symbol} ACTIVE_SNIPER with confidence=${derivedState.confidence}% (< 60%)`);
  }
  
  // Invariant 2: ACTIVE_CONFIRMED must have ignition >= 75
  if (lifecycleState === "ACTIVE_CONFIRMED" && derivedState.confidence < 75) {
    violations.push(`[INV] ${symbol} ACTIVE_CONFIRMED with confidence=${derivedState.confidence}% (< 75%)`);
  }
  
  // Invariant 3: BUILDING must NOT have trade plan
  if (lifecycleState === "BUILDING" && derivedState.targets) {
    violations.push(`[INV] ${symbol} BUILDING state has trade plan (TP/SL present)`);
  }
  
  // Invariant 4: ACTIVE_SNIPER/ACTIVE_CONFIRMED must have trade plan
  if ((lifecycleState === "ACTIVE_SNIPER" || lifecycleState === "ACTIVE_CONFIRMED") && !derivedState.targets) {
    violations.push(`[INV] ${symbol} ${lifecycleState} missing trade plan`);
  }
  
  // Invariant 5: ACTIVE_* must have LONG or SHORT direction
  if ((lifecycleState === "ACTIVE_SNIPER" || lifecycleState === "ACTIVE_CONFIRMED") && derivedState.direction === "NONE") {
    violations.push(`[INV] ${symbol} ${lifecycleState} with NONE direction`);
  }
  
  // Invariant 6: EXPIRED/INVALIDATED must NOT be renderable
  if ((lifecycleState === "EXPIRED" || lifecycleState === "INVALIDATED") && renderable) {
    violations.push(`[INV] ${symbol} ${lifecycleState} marked as renderable (should be false)`);
  }
  
  // Invariant 7: EXPIRED/INVALIDATED must NOT have active trade
  if ((lifecycleState === "EXPIRED" || lifecycleState === "INVALIDATED") && hasActiveTrade) {
    violations.push(`[INV] ${symbol} ${lifecycleState} has active trade (should be false)`);
  }
  
  // Invariant 8: renderable=true only for BUILDING/ACTIVE_*
  if (renderable && !["BUILDING", "ACTIVE_SNIPER", "ACTIVE_CONFIRMED"].includes(lifecycleState)) {
    violations.push(`[INV] ${symbol} ${lifecycleState} renderable=true (invalid state for rendering)`);
  }
