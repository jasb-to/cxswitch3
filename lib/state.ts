...
export async function runMigrationIfNeeded(): Promise<void> {
  ...
  for (const s of legacySignals) {
    ...
    const entry: SignalHistoryEntry = {
      id:s.id,pair:s.pair,direction:s.direction,type:s.type,entry:s.entry,stop:s.stop,target:stages.tp2 ?? s.target,
      tp1:stages.tp1,tp2:stages.tp2,tp3:stages.tp3,timestamp:s.timestamp,rr:s.rr ?? 0,status,
      exitReason:s.exitReason || s.meta?.status,exitPrice:s.exitPrice,exitTimestamp:s.exitTimestamp,context:s.context || {},version:1,
    };
    historyEntries.push(entry);
    if (!latestAlerts[entry.pair] || entry.timestamp > latestAlerts[entry.pair].timestamp) latestAlerts[entry.pair] = entry;
    if (status === "ACTIVE" && s.meta?.status === "ACTIVE" && !s.exited) {
      activeSignals.push({id:entry.id,pair:entry.pair,direction:entry.direction,type:entry.type,entry:entry.entry,stop:entry.stop,target:entry.target,tp1:entry.tp1,tp2:entry.tp2,tp3:entry.tp3,timestamp:entry.timestamp,rr:entry.rr,status:"ACTIVE",context:entry.context,version:1});
    }
  }
...
export async function updateActiveTradeMilestones(id:string,price:number):Promise<ActiveTrade|undefined>{
  const active=await getActiveSignals();const trade=active.find(a=>a.id===id);if(!trade)return undefined;
  const hit=(level:number|undefined,direction:"LONG"|"SHORT")=>level!==undefined&&(direction==="LONG"?price>=level:price<=level);let changed=false;
  if(!trade.tp1HitAt&&hit(trade.tp1,trade.direction)){trade.tp1HitAt=Date.now();changed=true;console.log(`[MILESTONE] ${trade.pair} — TP1 reached @ ${price}`);}
  if(!trade.tp2HitAt&&hit(trade.tp2,trade.direction)){trade.tp2HitAt=Date.now();changed=true;console.log(`[MILESTONE] ${trade.pair} — TP2 reached @ ${price}`);}
  if(!trade.tp3HitAt&&hit(trade.tp3,trade.direction)){trade.tp3HitAt=Date.now();changed=true;console.log(`[MILESTONE] ${trade.pair} — TP3 reached @ ${price}`);}
  if(changed)await setActiveSignals(active);return trade;
}
...
// Latest alert is deliberately separate from active position state and full history.
// It means each market can always show the most recent alert that CXSwitch actually sent,
// even after that trade has closed. History remains the audit trail; activeSignals remains live position state.
export async function getLatestAlerts():Promise<Record<string,SignalHistoryEntry>>{
  ...
  if(latest&&Object.keys(latest).length)return latest;
  const history=await getSignalHistory();const derived:Record<string,SignalHistoryEntry>={};
  for(const h of history)if(!derived[h.pair]||h.timestamp>derived[h.pair].timestamp)derived[h.pair]=h;
  if(Object.keys(derived).length)await redis.set(LATEST_ALERTS_KEY,derived);return derived;
}

export async function appendSignalHistory(signal:Signal):Promise<void>{
  const history=await getSignalHistory();if(history.some(h=>h.id===signal.id)){console.log(`[HISTORY] Signal ${signal.id} already recorded`);return;}
  const entry:SignalHistoryEntry={id:signal.id,pair:signal.pair,direction:signal.direction,type:signal.type,entry:signal.entry,stop:signal.stop,target:signal.tp2 ?? signal.target,tp1:signal.tp1,tp2:signal.tp2,tp3:signal.tp3,timestamp:signal.timestamp,rr:signal.rr,status:"ACTIVE",context:signal.context,version:1};
  history.push(entry);if(history.length>500)history.splice(0,history.length-500);await setSignalHistory(history);
  const latest=await getLatestAlerts();latest[entry.pair]=entry;await redis.set(LATEST_ALERTS_KEY,latest);
  console.log(`[HISTORY] Appended ${signal.pair} ${signal.direction} ${signal.type} | TP1 ${entry.tp1 ?? "—"} | TP2 ${entry.tp2 ?? "—"} | TP3 ${entry.tp3 ?? "—"} | latest alert persisted`);
}

export async function updateSignalHistoryStatus(id:string,status:HistoryStatus,exitReason?:string,exitPrice?:number):Promise<void>{
  const history=await getSignalHistory();const idx=history.findIndex(h=>h.id===id);if(idx<0){console.log(`[HISTORY] Warning: could not find ${id}`);return;}
  history[idx].status=status;if(exitReason)history[idx].exitReason=exitReason;if(exitPrice!==undefined)history[idx].exitPrice=exitPrice;history[idx].exitTimestamp=Date.now();await setSignalHistory(history);
  const latest=await getLatestAlerts();if(latest[history[idx].pair]?.id===id)latest[history[idx].pair]=history[idx];await redis.set(LATEST_ALERTS_KEY,latest);
...
export async function updateHistoryMilestones(id:string,price:number):Promise<SignalHistoryEntry|undefined>{
  const history=await getSignalHistory();const h=history.find(x=>x.id===id);if(!h)return undefined;const hit=(level:number|undefined,direction:"LONG"|"SHORT")=>level!==undefined&&(direction==="LONG"?price>=level:price<=level);let changed=false;
  if(!h.tp1HitAt&&hit(h.tp1,h.direction)){h.tp1HitAt=Date.now();changed=true;}if(!h.tp2HitAt&&hit(h.tp2,h.direction)){h.tp2HitAt=Date.now();changed=true;}if(!h.tp3HitAt&&hit(h.tp3,h.direction)){h.tp3HitAt=Date.now();changed=true;}
  if(changed){await setSignalHistory(history);const latest=await getLatestAlerts();if(latest[h.pair]?.id===id)latest[h.pair]=h;await redis.set(LATEST_ALERTS_KEY,latest);}return h;
}
