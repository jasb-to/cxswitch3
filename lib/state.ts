// lib/state.ts — canonical CXSwitch state
// ============================================================
// activeSignals  → currently tracked manual trades
// signalHistory  → permanent alert history used by the dashboard
// cooldowns      → post-exit re-entry blocks
// market / cron / snapshot → dashboard diagnostics
...
export async function runMigrationIfNeeded(): Promise<void> {
  ...
  console.log("[STATE] Running legacy state migration...");
  ...
  for (const s of legacySignals) {
    ...
    historyEntries.push({
      id: s.id, pair: s.pair, direction: s.direction, type: s.type,
      entry: s.entry, stop: s.stop, target: stages.tp2 ?? s.target,
      tp1: stages.tp1, tp2: stages.tp2, tp3: stages.tp3,
      timestamp: s.timestamp, rr: s.rr ?? 0, status: historyStatus,
      exitReason: s.exitReason || s.meta?.status, exitPrice: s.exitPrice,
      exitTimestamp: s.exitTimestamp, context: s.context || {}, version: s.version ?? 54,
    });
    ...
    if (isActive) {
      activeSignals.push({
        ...
        entry: s.entry, stop: s.stop, target: stages.tp2 ?? s.target,
        tp1: stages.tp1, tp2: stages.tp2, tp3: stages.tp3,
        timestamp: s.timestamp, rr: s.rr ?? 0, status: "ACTIVE",
        context: s.context || {}, version: s.version ?? 54,
      });
    }
  }
  ...
  for (const [key, t] of Object.entries(legacyTrades)) {
    ...
    activeSignals.push({
      id: t.id, pair: t.pair || key.split("_")[0], direction: t.direction,
      type: t.type || "ENTRY_1", entry: t.entry, stop: t.stop,
      target: stages.tp2 ?? t.target, tp1: stages.tp1, tp2: stages.tp2, tp3: stages.tp3,
      timestamp: t.timestamp, rr: 0, status: "ACTIVE", context: t.context || {}, version: t.version ?? 54,
    });
  }
...
export async function addActiveSignal(signal: Signal): Promise<void> {
  ...
  const trade: ActiveTrade = {
    id: signal.id, pair: signal.pair, direction: signal.direction, type: signal.type,
    entry: signal.entry, stop: signal.stop, target: signal.tp2 ?? signal.target,
    tp1: signal.tp1, tp2: signal.tp2, tp3: signal.tp3,
    timestamp: signal.timestamp, rr: signal.rr, status: "ACTIVE",
    context: signal.context, version: signal.version,
  };
...
export async function removeActiveSignal(pair: string, direction: "LONG" | "SHORT"): Promise<void> {
  ...
  if (filtered.length !== active.length) { await setActiveSignals(filtered); console.log(`[ACTIVE] Removed ${pair} ${direction}`); }
}

export async function removeActiveSignalById(id: string): Promise<void> {
  const active = await getActiveSignals();
  const filtered = active.filter(a => a.id !== id);
  if (filtered.length !== active.length) { await setActiveSignals(filtered); console.log(`[ACTIVE] Removed signal ${id}`); }
}
...
export async function updateActiveTradeMilestones(id: string, price: number): Promise<ActiveTrade | undefined> {
  ...
  const hit = (level: number | undefined, direction: "LONG" | "SHORT") => level !== undefined && (direction === "LONG" ? price >= level : price <= level);
  let changed = false;
...
export async function appendSignalHistory(signal: Signal): Promise<void> {
  ...
  if (history.some(h => h.id === signal.id)) { console.log(`[HISTORY] Signal ${signal.id} already recorded`); return; }
...
export async function updateSignalHistoryStatus(id: string,status: HistoryStatus,exitReason?: string,exitPrice?: number): Promise<void> {
  const history = await getSignalHistory(); const idx = history.findIndex(h => h.id === id);
  if (idx < 0) { console.log(`[HISTORY] Warning: could not find ${id}`); return; }
...
export async function updateHistoryMilestones(id: string, price: number): Promise<SignalHistoryEntry | undefined> {
  const history = await getSignalHistory(); const h = history.find(x => x.id === id); if (!h) return undefined;
  const hit = (level: number | undefined, direction: "LONG" | "SHORT") => level !== undefined && (direction === "LONG" ? price >= level : price <= level);
  let changed = false;
  if (!h.tp1HitAt && hit(h.tp1, h.direction)) { h.tp1HitAt = Date.now(); changed = true; }
  if (!h.tp2HitAt && hit(h.tp2, h.direction)) { h.tp2HitAt = Date.now(); changed = true; }
  if (!h.tp3HitAt && hit(h.tp3, h.direction)) { h.tp3HitAt = Date.now(); changed = true; }
...
// Legacy API shims retained only where older routes still import them.
export async function getSignals(): Promise<Signal[]> { const active=await getActiveSignals(); return active.map(a=>({...a,scale:a.type,adx:0,rsi:0,stochK:0,stochD:0,expectedMove:0,reason:"",trend:a.direction,location:"",trigger:""} as Signal)); }
export async function setSignals(_: Signal[]): Promise<void> {}
export async function getActiveTrades(): Promise<Record<string,any>> { const active=await getActiveSignals(); const out:Record<string,any>={}; for(const a of active) out[`${a.pair}_${a.direction}`]={...a,crossHash:a.context?.crossHash||""}; return out; }
export async function setActiveTrades(_:Record<string,any>): Promise<void> {}
export async function addSignalToHistory(signal:Signal,reason:string,exitPrice:number):Promise<void>{await updateSignalHistoryStatus(signal.id,"FAILED",reason,exitPrice);}
export async function persistExit(_:any):Promise<void>{}
export async function persistLastExit(_:string,__:any):Promise<void>{}
export async function loadLastExit(_:string):Promise<any|null>{return null;}
