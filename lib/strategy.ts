...
function snapshot(pair:string,c:Candle[],d:"LONG"|"SHORT",tl:TL,price:number,dailyLive?:DailyLiveContext,lastBreakout?:BreakoutRecord){
 const closes=c.map(x=>x.close),st=stochRsi(closes),r=rsi(closes),a=adx(c),e8=ema(closes,TF_FAST).at(-1)!,e21=ema(closes,TF_SLOW).at(-1)!,d1=getDaily513Diagnostic(c),m=macd4h(c);
 const dist=tl.valid?(price-tl.price)/tl.price;
 const closed=c.length>1?c.slice(0,-1):c,entryLast=closed.at(-1)!,entryPrev=closed.at(-2)??entryLast,entrySt=stochRsi(closed.map(x=>x.close));
 const nearThreshold=Math.max(0,RETEST_PCT-0.0025);
 const longEntryTL=buildTrendline(closed,"LONG",60),shortEntryTL=buildTrendline(closed,"SHORT",60);
 const longNearTL=longEntryTL.valid&&Math.abs((entryLast.close-longEntryTL.price)/Math.max(Math.abs(longEntryTL.price),1))<=nearThreshold;
 const shortNearTL=shortEntryTL.valid&&Math.abs((entryLast.close-shortEntryTL.price)/Math.max(Math.abs(shortEntryTL.price),1))<=nearThreshold;
 const longStochExtreme=entrySt.k<20,longStochTurn=entrySt.k>entrySt.d&&entrySt.k<40,shortStochTurn=entrySt.k<entrySt.d&&entrySt.k>60;
 const longRejection=entryLast.close>entryLast.open&&entryLast.close>entryPrev.close,shortRejection=entryLast.close<entryLast.open&&entryLast.close<entryPrev.close;
 const entry1LongReady=longNearTL&&(longStochExtreme||(longStochTurn&&longRejection)),entry1ShortReady=shortNearTL&&shortStochTurn&&shortRejection;
 const entry2Check=(dir:"LONG"|"SHORT",trendline:TL)=>{if(!trendline.valid||!lastBreakout||lastBreakout.direction!==dir)return false;const i=c.length-1,line=lineAt(trendline,i),recent=i-lastBreakout.candleIndex;if(recent<0||recent>BREAKOUT_EXPIRY_CANDLES)return false;const retest=dir==="LONG"?c.at(-1)!.low<=line*(1+RETEST_PCT)&&c.at(-1)!.close>line&&c.at(-1)!.close>=c.at(-2)!.close:c.at(-1)!.high>=line*(1-RETEST_PCT)&&c.at(-1)!.close<line&&c.at(-1)!.close<=c.at(-2)!.close;const distance=Math.abs(price-lastBreakout.price)/Math.max(Math.abs(lastBreakout.price),1);return retest&&distance<=0.01;};
 const entry2LongReady=entry2Check("LONG",buildTrendline(c,"LONG",60)),entry2ShortReady=entry2Check("SHORT",buildTrendline(c,"SHORT",60));
 return{pair,price:round(price),timestamp:Date.now(),trend:d+" "+strength(daily(c),d),location:dist===null?"NO_TL":"NEAR_TL",trigger:"WAITING",adx:a,rsi:Math.round(r*10)/10,stochK:st.k,stochD:st.d,trendlinePrice:tl.valid?round(tl.price):0,distToTrendline:dist===null?null:Math.round(Math.abs(dist)*10000)/100,ema8_4h:round(e8),ema21_4h:round(e21),fourH513:get4HEmaDiagnostic(c),macd4h:m,daily513:d1,dailyLive:dailyLive||null,momentumState:d==="LONG"?(r>=80?"OVEREXTENDED":r>=70?"HOT":st.k<20?"PULLBACK":"NEUTRAL"):(r<=20?"OVEREXTENDED":r<=30?"HOT":st.k>80?"PULLBACK":"NEUTRAL"),trendlineStatus:tl.stale?"STALE_REBUILD":tl.valid?"ACTIVE":"REBUILDING",trendlineReason:tl.reason,trendlinePivots:tl.pivots.length,trendlineAgeCandles:tl.ageCandles,trendlineSlope:round(tl.slope),trendlineStaleByAge:tl.staleByAge,trendlineStaleByDistance:tl.staleByDistance,
  entry1Closed4hTimestamp:entryLast.timestamp,entry1LongNearTL:longNearTL,entry1ShortNearTL:shortNearTL,entry1LongStochTurn:longStochTurn,entry1ShortStochTurn:shortStochTurn,entry1LongRejection:longRejection,entry1ShortRejection:shortRejection,entry1LongReady,entry1ShortReady,entry1NearTLThresholdPct:nearThreshold*100,entry1LongTrendlinePrice:longEntryTL.valid?round(longEntryTL.price):0,entry1ShortTrendlinePrice:shortEntryTL.valid?round(shortEntryTL.price):0,entry1ClosedStochK:entrySt.k,entry1ClosedStochD:entrySt.d,entry2LongReady,entry2ShortReady};}
...
export function generateSignal(pair:string,candles1h:Candle[],candles4h:Candle[],candles15m:Candle[],activeTrades?:any[],currentPrice?:number,lastBreakout?:BreakoutRecord,dailyLive?:DailyLiveContext):SignalResult{
 ...
 if(!tl.valid&&!early){debug.push(`State: ${tl.reason}`);return{market:snapshot(pair,candles4h,dir,tl,price,dailyLive,lastBreakout),debug};}
 let type:"ENTRY_1"|"ENTRY_2"|"ADD"|null=null;
 if(early){type="ENTRY_1";debug.push(`[V28 EARLY] ${pair} ${dir} | closed 4H trendline location + StochRSI timing`);}
 else if(breakout){type="ENTRY_1";}
 else if(retest){if(has){type="ADD";}else{const hasRecentBreakout=!!lastBreakout&&lastBreakout.direction===dir&&(i-lastBreakout.candleIndex)>=0&&(i-lastBreakout.candleIndex)<=BREAKOUT_EXPIRY_CANDLES;if(!hasRecentBreakout)return{market:snapshot(pair,candles4h,dir,tl,price,dailyLive,lastBreakout),debug};const retestDistance=Math.abs(price-lastBreakout!.price)/Math.max(Math.abs(lastBreakout!.price),1);if(retestDistance>=0.01)return{market:snapshot(pair,candles4h,dir,tl,price,dailyLive,lastBreakout),debug};type="ENTRY_2";}}
   ...
   if(has){type="ADD";debug.push(`[V28 CONTINUATION] ${pair} ${dir} | controlled 4H pullback into 8/21`);}
     ...
     if(!hasRecentBreakout)return{market:snapshot(pair,candles4h,dir,tl,price,dailyLive,lastBreakout),debug};
     ...
     if(!nearBreakout)return{market:snapshot(pair,candles4h,dir,tl,price,dailyLive,lastBreakout),debug};
     type="ENTRY_2";debug.push(`[V28 CONTINUATION] ${pair} ${dir} | ENTRY_2 after controlled 4H pullback`);
   }
 }
 if(!type){debug.push(`State: ${has?"POST_BREAKOUT_WAIT":"WAITING"}`);return{market:snapshot(pair,candles4h,dir,tl,price,dailyLive,lastBreakout),debug};}
 ...
 const tp1=dir==="LONG"?entry+risk:entry-risk,tp2=dir==="LONG"?entry+risk*1.5:entry-risk*1.5,tp3=dir==="LONG"?entry+risk*2:entry-risk*2,target=tp2,rr=1.5;if(rr<MIN_RR)return{market:snapshot(pair,candles4h,dir,tl,price,dailyLive,lastBreakout),debug:[...debug,"R:R below minimum"]};
 ...
 const s:Signal={id:`${pair}_${type}_${now}`,pair,direction:dir,type,scale:type,entry:round(entry),stop:round(stop),target:round(target),tp1:round(tp1),tp2:round(tp2),tp3:round(tp3),confidence:early?70:type==="ENTRY_1"?80:type==="ENTRY_2"?70:85,rr,adx:a,rsi:r,stochK:st.k,stochD:st.d,expectedMove:Math.round(Math.abs(tp3-entry)/entry*1000)/10,reason:early?`${dir} ENTRY_1 EARLY ${earlyGrade} | 1D ${dailyState||dailyCandidate||"REGIME"} | closed 4H Stoch ${entrySt.k}/${entrySt.d} | near trendline + rejection timing`: `${dir} ${type} | 4H trendline breakout/retest | 1D ${contextDir?strength(d,contextDir):"NEUTRAL"} | Stoch ${st.k}/${st.d}`,timestamp:now,version:CURRENT_SIGNAL_VERSION,trend:`${dir} ${strength(d,dir)}`,location,trigger,context:{marketPhase:early?`${dir} EARLY REVERSAL ${earlyGrade}`:`${dir} ${strength(d,dir)}`,structure:early?`4H ${structure.structure||"TRANSITION"} + closed-candle reversal timing`:(breakout?"4H TRENDLINE BREAKOUT":"4H BREAKOUT RETEST"),momentum:`RSI ${r} | Stoch ${st.k}/${st.d} | MACD hist ${round(macd.histogram)}`,pullback:continuation?"controlled_4h_momentum_pullback":retest?"confirmed_retest":early?"early_transition":"breakout",fourH513,daily513,dailyLive:dailyLive||null,macd4h:macd,trendAlignment,sizeMultiplier:riskMultiplier,earlyGrade:early?earlyGrade:undefined,risk:{baseRisk:round(baseRisk),positionSize:round(positionSize),trendAlignment,sizeMultiplier:riskMultiplier,riskMultiplier,estimatedLiquidation:round(l),safeBoundary:round(safe),leverage:LEVERAGE},breakoutRecord:breakoutRecord?{direction:breakoutRecord.direction,price:round(breakoutRecord.price),timestamp:breakoutRecord.timestamp,candleIndex:breakoutRecord.candleIndex}:undefined,stages:{tp1:round(tp1),tp2:round(tp2),tp3:round(tp3),tp1R:1,tp2R:1.5,tp3R:2}}};
 debug.push(`SIGNAL: ${type} ${dir} @ ${s.entry} | SL ${s.stop} | TP1 ${s.tp1} | TP2 ${s.tp2} | TP3 ${s.tp3} | ${trendAlignment} | ${early?`EARLY_${earlyGrade}`:"CONFIRMED"} | size x${riskMultiplier}`);return{signal:s,signals:[s],market:snapshot(pair,candles4h,dir,tl,price,dailyLive,lastBreakout),debug};
}
