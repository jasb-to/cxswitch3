// lib/strategy.ts — v37.4 "Setup Detector" (v28 entry logic + v37 risk management)
// ============================================================
// ENTRY: v28 logic — near TL + stoch for ENTRY_1/2, momentum for ADD
// EXIT: Stoch extreme opposite + trend reversal
// RISK: v37 stop trail, profit locks, position phases
// TRENDLINE: 3+ pivots, any R² (logged, not gated)
// ADX: 4H only, >20 for ADD momentum (not general gating)
...
// --- STATEFUL TRENDLINE (v28: 3+ pivots, any R²) ---
function getTrendline(pair: string, candles: Candle[], direction: "LONG" | "SHORT"): { price: number; r2: number; age: number; touches: number } | null {
  ...
  if (existing && existing.direction === direction && (now - existing.lastUpdated) < maxAge) {
    ...
    if (deviation < 0.02) {
      ...
      return { price, r2: 0.85, age: now - existing.lastUpdated, touches: existing.pivots.length };
    }
  }

  // Calculate new trendline
  ...
  return { price, r2: Math.round(r2 * 100) / 100, age: 0, touches: n };
}
...
// --- MAIN SIGNAL (v28 entry logic restored) ---
export function generateSignal(
  ...
  debug.push(`[TREND] 1D ${t1d.direction || "NONE"} ${t1d.strength}`);
  ...
  // Trendline: 3+ pivots, any R² (v28 style)
  const trendline = getTrendline(pair, candles4h, t1d.direction);
  if (!trendline) {
    debug.push("[TL] No trendline — need 3+ pivots");
    return { debug };
  }
  ...
  debug.push(`[TL] Price=${tlPrice.toFixed(2)} | R²=${trendline.r2} | Touches=${trendline.touches} | DistFromPrice=${(dist*100).toFixed(2)}%`);

  // StochRSI on 4H
  const stoch = stochRsi(candles4h.map(c => c.close));
  debug.push(`[STOCH] 4H K=${stoch.k} D=${stoch.d}`);
  ...
  const ema8Val = ema8_4h[ema8_4h.length - 1];
  const ema21Val = ema21_4h[ema21_4h.length - 1];

  // 4H ADX (not 1D)
  const adx4h = adx(candles4h);
  debug.push(`[ADX] 4H ADX=${adx4h.toFixed(1)}`);

  // --- ENTRY CONDITIONS (v28 logic) ---

  // ENTRY_1/2 conditions (no volume, no ADX, no EMA alignment required)
  ...
  // ADD conditions (momentum filters live here)
  ...
  const emaAligned = t1d.direction === "LONG"
    ? price > ema8Val && price > ema21Val
    : price < ema8Val && price < ema21Val;
  const stochMomentum = t1d.direction === "LONG" ? stoch.k > stoch.d : stoch.k < stoch.d;
  const adxStrong = adx4h > 20; // 4H ADX, threshold 20

  debug.push(`[ENTRY] nearTL=${nearTrendline} beyondTL=${beyondTrendline} stochExtreme=${stochExtreme} stochTurning=${stochTurning} confirming=${confirming} emaAligned=${emaAligned} volUp=${volUp} stochMomentum=${stochMomentum} adxStrong=${adxStrong}`);

  // Determine raw signal type (v28 logic)
  let rawType: "ENTRY_1" | "ENTRY_2" | "ADD" | null = null;

  if (nearTrendline && stochExtreme) {
    rawType = "ENTRY_1";
    debug.push(`[SIGNAL] ENTRY_1: near TL + stoch extreme`);
  } else if (nearTrendline && stochTurning && !stochExtreme) {
    rawType = "ENTRY_2";
    debug.push(`[SIGNAL] ENTRY_2: near TL + stoch turning`);
    ...
    if (volUp || stochMomentum || adxStrong) {
      ...
      debug.push(`[SIGNAL] ADD: beyond TL + confirming + aligned + momentum(${momentumParts.join("+")})`);
    } else {
    ...
    debug.push(`[ENTRY] No signal: ${stateParts.join(" | ")}`);
  }
  ...
  if (hyst.lastSignalType && finalType === hyst.lastSignalType) {
    ...
    if (priceMove < HYSTERESIS_BAND) {
      debug.push(`[HYST] Lock: ${finalType} | move ${(priceMove * 100).toFixed(2)}% < ${(HYSTERESIS_BAND * 100).toFixed(2)}%`);
      return { debug };
    }
  }
  ...
  if (rr < MIN_RR) {
    debug.push(`[SIGNAL] R:R ${rr.toFixed(2)} < ${MIN_RR}`);
    return { debug };
  }
  ...
  const market = {
    ...
    ema8: Math.round(ema8Val * 100) / 100,
...
export function getMarketSnapshot(
  ...
  return {
    ...
    r2: trendline ? trendline.r2 : null,
    touches: trendline ? trendline.touches : 0,
  };
}
...
export function shouldHold(signal: Signal, candles4h: Candle[], currentPrice: number, now?: number): HoldResult {
  ...
  // Exit when Stoch hits extreme opposite
  const closes4h = candles4h.map(c => c.close);
