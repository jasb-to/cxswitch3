// lib/strategy.ts — v57 "v28 core + 5/13 fast bias"
// ============================================================
// The v28 entry architecture is kept deliberately simple:
...
//
// Daily bias remains 5/13 because that is intentionally faster than 8/21.
// No exchange state, gating framework or extra scoring layer belongs here.
...
export interface Signal {
  ...
  tp1?:number; tp3?:number; confidence:number; rr:number; adx:number; rsi:number;
...
export const CURRENT_SIGNAL_VERSION = 57;
...
function rsi(a:number[],p=14){let g=0,l=0;for(let i=1;i<=p&&i<a.length;i++){const d=a[a.length-i]-a[a.length-i-1];if(d>0)g+=d;else l+=Math.abs(d);}const ag=g/p,al=l/p;if(al===0)return 100;return 100-100/(1+ag/al);}
...
function samePosition(pair:string,d:"LONG"|"SHORT",trades:any[]|undefined){return !!trades?.some(t=>(t.pair===pair||t.symbol===pair)&&t.direction===d);}
...
export function generateSignal(pair:string,candles1h:Candle[],candles4h:Candle[],candles15m:Candle[],activeTrades?:any[],currentPrice?:number):SignalResult{
  ...
  // ADD is intentionally a valid alert even without a tracked position: a breakout is itself a tradable early opportunity.
  const atrVal=atr(candles4h),lows=candles4h.slice(-20).map(x=>x.low),highs=candles4h.slice(-20).map(x=>x.high),entry=price;
  const isEntry=raw!=="ADD",riskStop=dir==="LONG"?Math.min(Math.min(...lows),entry-atrVal*(isEntry?ENTRY_ATR:ADD_ATR)):Math.max(Math.max(...highs),entry+atrVal*(isEntry?ENTRY_ATR:ADD_ATR));
  const risk=Math.abs(entry-riskStop);if(!risk){debug.push("Zero risk");return{debug};}
  const tp1=dir==="LONG"?entry+risk*2:entry-risk*2,tp3=dir==="LONG"?entry+risk*6:entry-risk*6;
  const structure=dir==="LONG"?Math.max(...highs):Math.min(...lows),target=dir==="LONG"?Math.max(tp1,Math.min(structure,entry+atrVal*5)):Math.min(tp1,Math.max(structure,entry-atrVal*5));
  ...
  const s:Signal={id:`${pair}_${now}`,pair,direction:dir,type:raw,scale:raw,entry:Math.round(entry*100)/100,stop:Math.round(riskStop*100)/100,target:Math.round(target*100)/100,tp1:Math.round(tp1*100)/100,tp3:Math.round(tp3*100)/100,confidence:raw==="ENTRY_1"?55:raw==="ENTRY_2"?65:85,rr:Math.round(rr*100)/100,adx:adxVal,rsi:Math.round(rsi(closes)*10)/10,stochK:st.k,stochD:st.d,expectedMove:Math.round(Math.abs(target-entry)/entry*1000)/10,reason:`${dir} ${raw} | 1D ${strength(d1,dir)} | ${near?"TL approach":"breakout"} | Stoch ${st.k}/${st.d} | RR ${rr.toFixed(2)}`,timestamp:now,version:CURRENT_SIGNAL_VERSION,trend:`${dir} ${strength(d1,dir)}`,location:near?"NEAR_TL":"BEYOND_TL",trigger:"FIRED",context:{marketPhase:`${dir} ${strength(d1,dir)}`,structure:"4H trendline",momentum:`Stoch ${st.k}/${st.d}`,pullback:near?"active":"breakout"}};
  debug.push(`SIGNAL: ${raw} ${dir} @ ${s.entry} TP ${s.target} SL ${s.stop} RR ${s.rr}`);
...
export function shouldHold(s:Signal,c:Candle[],p:number):HoldResult{const d=daily(c),dir=bias(d),st=stochRsi(c.map(x=>x.close));if(dir&&dir!==s.direction&&((s.direction==="LONG"&&p<=s.entry)||(s.direction==="SHORT"&&p>=s.entry)))return{shouldHold:false,reason:"trend_reversed_unprofitable"};if((s.direction==="LONG"&&st.k>80)||(s.direction==="SHORT"&&st.k<20))return{shouldHold:false,reason:"stoch_extreme_opposite_exit"};const risk=Math.abs(s.entry-s.stop);if(risk){const r=s.direction==="LONG"?(p-s.entry)/risk:(s.entry-p)/risk;if(r>=2&&s.tp1)return{shouldHold:true,reason:"tp1_hit_scale_out_50",newStop:s.entry,scaleOut:{level:s.tp1,size:.5,label:"TP1"}};if(r>=1.5)return{shouldHold:true,reason:"be_lock_1_5r",newStop:s.entry};}return{shouldHold:isSignalStillValid(s,p).valid,reason:isSignalStillValid(s,p).reason};}
...
export function checkTradeStatus(s:Signal,p:number,now=Date.now()):TradeStatus{const v=isSignalStillValid(s,p,now);if(v.reason==="expired_ttl")return"EXPIRED";if(s.direction==="LONG"){if(p>=s.target)return"TP_HIT";if(p<=s.stop)return"SL_HIT";}else{if(p<=s.target)return"TP_HIT";if(p>=s.stop)return"SL_HIT";}return"ACTIVE";}
