// lib/strategy.ts — CXSwitch V28 entry architecture
// ============================================================
// V28 ENTRY_1 / ENTRY_2 architecture is preserved.
// ADD is execution-gated: existing same-direction position + confirmed TL retest + non-overextended momentum.
...
const ENTRY_MIN_RISK_PCT=0.008, ENTRY_MAX_RISK_PCT=0.035;
const ADD_MIN_RISK_PCT=0.008, ADD_MAX_RISK_PCT=0.025;
const STOP_EXECUTION_BUFFER=0.01;
const ADD_MAX_RSI_LONG=80, ADD_MIN_RSI_SHORT=20;
const ADD_MAX_STOCH_LONG=85, ADD_MIN_STOCH_SHORT=15;
// 3% is an eligibility window, not a trigger. The 4H candle still has to wick back through the TL.
const ADD_MAX_TL_DISTANCE=0.03;
const ADD_RETEST_WICK_PCT=0.015;

// Trendline lifecycle controls. These affect only which structural TL is considered active;
// V28 entry conditions, 5/13 bias, Stoch/RSI rules, retest percentages and ADD rules are unchanged.
const TL_LOOKBACK_CANDLES=60;
const TL_MAX_PIVOT_SPAN=72;
const TL_BREAK_PCT=0.005;
const TL_BREAK_ATR_MULT=0.35;
...
function pivots(c:Candle[],d:"LONG"|"SHORT"){const r:Pivot[]=[];for(let i=3;i<c.length-3;i++){const lo=c[i].low<c[i-1].low&&c[i].low<c[i-2].low&&c[i].low<c[i+1].low&&c[i].low<c[i+2].low;const hi=c[i].high>c[i-1].high&&c[i].high>c[i-2].high&&c[i].high>c[i+1].high&&c[i].high>c[i+2].high;if(d==="LONG"&&lo)r.push({index:i,price:c[i].low,timestamp:c[i].timestamp});if(d==="SHORT"&&hi)r.push({index:i,price:c[i].high,timestamp:c[i].timestamp});}return r;}
function fitTrendline(p:Pivot[],c:Candle[],d:"LONG"|"SHORT"):TrendlineResult{const n=p.length;if(n<3)return{valid:false,slope:0,intercept:0,price:0,pivots:p,ageCandles:0,reason:`No current ${d} structure — only ${n} confirmed pivots in recent window`,invalidated:false};const sx=p.reduce((s,x)=>s+x.index,0),sy=p.reduce((s,x)=>s+x.price,0),sxy=p.reduce((s,x)=>s+x.index*x.price,0),sx2=p.reduce((s,x)=>s+x.index*x.index,0),den=n*sx2-sx*sx;if(!den)return{valid:false,slope:0,intercept:0,price:0,pivots:p,ageCandles:0,reason:"Trendline candidate is degenerate",invalidated:false};const slope=(n*sxy-sx*sy)/den,intercept=(sy-slope*sx)/n,priceAt=(i:number)=>slope*i+intercept,price=priceAt(c.length-1),ageCandles=c.length-1-p[0].index,breakBuffer=Math.max(price*TL_BREAK_PCT,atr(c)*TL_BREAK_ATR_MULT),anchor=p.at(-1)!.index;for(let i=anchor+1;i<c.length;i++){const line=priceAt(i);if(d==="LONG"&&c[i].close<line-breakBuffer)return{valid:false,slope,intercept,price,pivots:p,ageCandles,reason:`Current ${d} TL invalidated — 4H close ${c[i].close.toFixed(2)} broke support ${line.toFixed(2)} by >${(breakBuffer/line*100).toFixed(2)}%`,invalidated:true};if(d==="SHORT"&&c[i].close>line+breakBuffer)return{valid:false,slope,intercept,price,pivots:p,ageCandles,reason:`Current ${d} TL invalidated — 4H close ${c[i].close.toFixed(2)} broke resistance ${line.toFixed(2)} by >${(breakBuffer/line*100).toFixed(2)}%`,invalidated:true};}return{valid:true,slope,intercept,price,pivots:p,ageCandles,reason:`${d} TL active`,invalidated:false};}
function trendline(c:Candle[],d:"LONG"|"SHORT"):TrendlineResult{const all=pivots(c,d),recent=all.filter(x=>x.index>=Math.max(0,c.length-TL_LOOKBACK_CANDLES));if(recent.length<3)return{valid:false,slope:0,intercept:0,price:0,pivots:recent,ageCandles:recent.length?c.length-1-recent[0].index:0,reason:`No current ${d} TL — ${recent.length}/3 confirmed pivots inside ${TL_LOOKBACK_CANDLES} candles`,invalidated:false};let p=recent.slice(-5);const latest=p.at(-1)!.index;p=p.filter(x=>x.index>=latest-TL_MAX_PIVOT_SPAN);return fitTrendline(p,c,d);}
...
function assetFilter(pair:string,d:"LONG"|"SHORT",type:"ENTRY_1"|"ENTRY_2"|"ADD"){if(type==="ADD")return true;if((pair==="HYPE"||pair==="SOL")&&d==="SHORT")return false;if(pair==="ETH"&&type==="ENTRY_2")return false;return true;}
...
export function generateSignal(pair:string,candles1h:Candle[],candles4h:Candle[],candles15m:Candle[],activeTrades?:any[],currentPrice?:number):SignalResult{
 ...
 const tl=trendline(candles4h,dir);debug.push(`[TL] ${dir} | candidates=${tl.pivots.length} | age=${tl.ageCandles} candles | slope=${tl.slope.toFixed(5)} | ${tl.reason}`);if(!tl.valid){return{market:marketSnapshot(pair,candles4h,dir,tl,currentPrice??candles4h.at(-1)!.close),debug};}
 ...
 const rsiVal=Math.round(rsi(closes)*10)/10,turn=dir==="LONG"?st.k>st.d:st.k<st.d,extreme=dir==="LONG"?st.k<20:st.k>80;const confirm=dir==="LONG"?last.close>last.open&&last.close>prev.close:last.close<last.open&&last.close<prev.close;const vol=last.volume>avg(candles4h.slice(-10).map(x=>x.volume))*1.3;const aligned=dir==="LONG"?price>e8&&price>e21:price<e8&&price<e21;const momentum=dir==="LONG"?st.k>st.d:st.k<st.d;const adxVal=adx(candles4h);const ema513=get4HEmaDiagnostic(candles4h);
 ...
 debug.push(`1D: ${dir} ${strength(d1,dir)} | 4H 5/13: ${ema513.label} | TL: ${tl.price.toFixed(2)} | Price: ${price.toFixed(2)} | Dist: ${(dist*100).toFixed(2)}% | RSI: ${rsiVal} | Stoch: ${st.k}/${st.d}`);
 // Profitability overlays from the completed 114-trade audit. They do not alter ADD.
 // ENTRY_1 remains early on purpose. ENTRY_2 must agree with the intermediate 4H regime.
 if(raw!==null&&raw!=="ADD"){
   if(!assetFilter(pair,dir,raw)){debug.push(`FILTER: ${pair} ${dir} ${raw} blocked by asset/direction audit`);raw=null;}
   else if(dir==="SHORT"&&ema513.direction!=="BEARISH"){debug.push(`FILTER: SHORT blocked — 4H regime is ${ema513.label}; short qualification requires bearish 4H 5/13`);raw=null;}
   else if(raw==="ENTRY_2"&&dir==="LONG"&&ema513.direction==="BEARISH"){debug.push(`FILTER: LONG ENTRY_2 blocked — 4H regime is ${ema513.label}; wait for 4H recovery`);raw=null;}
 }
 if(raw==="ADD"){
   if(!hasPosition){debug.push("ADD blocked: no active same-direction position — ADD is never a standalone entry");raw=null;}
   else if(!retestConfirmed(candles4h,dir,tl.price,price)){debug.push(`ADD blocked: no confirmed TL retest (price distance ${(Math.abs(dist)*100).toFixed(2)}%, capture window ${ADD_MAX_TL_DISTANCE*100}%, wick ${ADD_RETEST_WICK_PCT*100}%)`);raw=null;}
 ...
 if(!raw){const state=near?"NEAR_TL":beyond?"BEYOND_TL":"FAR_FROM_TL";debug.push(`State: ${state} | No signal`);return{market:marketSnapshot(pair,candles4h,dir,tl,price,st,adxVal),debug};}
 ...
 if(!risk){debug.push("Zero risk");return{market:marketSnapshot(pair,candles4h,dir,tl,price,st,adxVal),debug};}
 const tp1=dir==="LONG"?entry+risk:entry-risk,tp2=dir==="LONG"?entry+risk*2:entry-risk*2,tp3=dir==="LONG"?entry+risk*3:entry-risk*3,target=tp2,rr=2,expectedMove=Math.abs(tp3-entry)/entry*100;
 const s:Signal={id:`${pair}_${now}`,pair,direction:dir,type:raw,scale:raw,entry:roundPrice(entry),stop:roundPrice(stop),target:roundPrice(target),tp1:roundPrice(tp1),tp2:roundPrice(tp2),tp3:roundPrice(tp3),confidence:raw==="ENTRY_1"?55:raw==="ENTRY_2"?65:80,rr,adx:adxVal,rsi:rsiVal,stochK:st.k,stochD:st.d,expectedMove:Math.round(expectedMove*10)/10,reason:`${dir} ${raw} | 1D ${strength(d1,dir)} | 4H 5/13 ${ema513.label} | ${isAdd?"confirmed TL retest":"TL approach"} | RSI ${rsiVal} | Stoch ${st.k}/${st.d} | TP1 ${tp1.toFixed(2)} | TP2 ${tp2.toFixed(2)} | TP3 ${tp3.toFixed(2)}`,timestamp:now,version:CURRENT_SIGNAL_VERSION,trend:`${dir} ${strength(d1,dir)}`,location:near?"NEAR_TL":"BEYOND_TL",trigger:"FIRED",context:{marketPhase:`${dir} ${strength(d1,dir)}`,structure:"4H trendline",momentum:`RSI ${rsiVal} | Stoch ${st.k}/${st.d}`,pullback:isAdd?"confirmed_retest":"active",fourH513:{stage:ema513.stage,label:ema513.label,direction:ema513.direction,spreadAtr:ema513.spreadAtr},exitPlan:{tp1Pct:40,tp2Pct:40,tp3Pct:20,afterTp1:"STOP_TO_BREAKEVEN",afterTp2:"STOP_TO_1R",runner:"TP3_OR_4H_EXHAUSTION"},filters:{shortQualification:dir!=="SHORT"||ema513.direction==="BEARISH",assetFilter:assetFilter(pair,dir,raw),entryRegime:raw!=="ENTRY_2"||dir!=="LONG"||ema513.direction!=="BEARISH"},risk:{safe:true,estimatedLiquidation:roundPrice(liquidation),safeBoundary:roundPrice(safeBoundary),structuralStop:roundPrice(structuralStop),executionStop:roundPrice(stop),leverage:EXECUTION_LEVERAGE,maxRiskPct:(isAdd?ADD_MAX_RISK_PCT:ENTRY_MAX_RISK_PCT)*100},stages:{tp1:roundPrice(tp1),tp2:roundPrice(tp2),tp3:roundPrice(tp3),tp1R:1,tp2R:2,tp3R:3}}};
 debug.push(`SIGNAL: ${raw} ${dir} @ ${s.entry} | 4H ${ema513.label} | SL ${s.stop} | TP1 ${s.tp1} | TP2 ${s.tp2} | TP3 ${s.tp3} | RR ${s.rr}`);
 return{signal:s,signals:[s],market:marketSnapshot(pair,candles4h,dir,tl,price,st,adxVal),debug};
}

function marketSnapshot(pair:string,c:Candle[],dir:"LONG"|"SHORT",tl:TrendlineResult,price:number,st?:{k:number;d:number},adxVal?:number){const closes=c.map(x=>x.close),near=tl.valid&&Math.abs((price-tl.price)/tl.price)<TL_THRESHOLD,beyond=tl.valid&&(dir==="LONG"?price>tl.price*1.008:price<tl.price*.992),e8=ema(closes,TF_FAST).at(-1)!,e21=ema(closes,TF_SLOW).at(-1)!,r=rsi(closes),stoch=st??stochRsi(closes),ema513=get4HEmaDiagnostic(c);let momentumState="NEUTRAL";if((dir==="LONG"&&r>=80)||(dir==="SHORT"&&r<=20))momentumState="OVEREXTENDED";else if((dir==="LONG"&&r>=70)||(dir==="SHORT"&&r<=30))momentumState="HOT";else if((dir==="LONG"&&stoch.k<20)||(dir==="SHORT"&&stoch.k>80))momentumState="PULLBACK";return{pair,price:Math.round(price*100)/100,timestamp:Date.now(),trend:`${dir} ${strength(daily(c),dir)}`,location:near?"NEAR_TL":beyond?"BEYOND_TL":tl.valid?"FAR_FROM_TL":"TL_INVALIDATED",trigger:near?"READY":"WAITING",adx:adxVal??adx(c),rsi:Math.round(r*10)/10,stochK:stoch.k,stochD:stoch.d,trendlinePrice:tl.valid?Math.round(tl.price*100)/100:0,distToTrendline:tl.valid?Math.round(Math.abs((price-tl.price)/tl.price)*10000)/100:null,ema8_4h:Math.round(e8*100)/100,ema21_4h:Math.round(e21*100)/100,fourH513:{stage:ema513.stage,label:ema513.label,direction:ema513.direction},momentumState,trendlineStatus:tl.valid?"ACTIVE":tl.invalidated?"INVALIDATED":"REBUILDING",trendlineReason:tl.reason,trendlinePivots:tl.pivots.length,trendlineAgeCandles:tl.ageCandles,trendlineSlope:Math.round(tl.slope*100000)/100000};}
export function getMarketSnapshot(pair:string,candles1h:Candle[],candles4h:Candle[],candles15m:Candle[]){const d=daily(candles4h),dir=bias(d),price=candles4h.at(-1)?.close||0;if(!dir)return{pair,price,timestamp:Date.now(),trend:"FLAT",location:"NONE",trigger:"NO_BIAS",adx:0,rsi:0,stochK:0,stochD:0,trendlinePrice:0,distToTrendline:0,momentumState:"NEUTRAL"};const tl=trendline(candles4h,dir);const st=stochRsi(candles4h.map(x=>x.close));const base=marketSnapshot(pair,candles4h,dir,tl,price,st,adx(candles4h));const estimatedLiquidation=estimateLiquidationPrice(price,dir),safeBoundary=liquidationSafeBoundary(price,dir);return{...base,risk:{estimatedLiquidation:roundPrice(estimatedLiquidation),safeBoundary:roundPrice(safeBoundary),leverage:EXECUTION_LEVERAGE}};}
...
export function shouldHold(s:Signal,c:Candle[],p:number):HoldResult{const d=daily(c),dir=bias(d),st=stochRsi(c.map(x=>x.close));if(dir&&dir!==s.direction&&((s.direction==="LONG"&&p<=s.entry)||(s.direction==="SHORT"&&p>=s.entry)))return{shouldHold:false,reason:"trend_reversed_unprofitable"};const risk=Math.abs(s.entry-s.stop);if(risk){const r=s.direction==="LONG"?(p-s.entry)/risk:(s.entry-p)/risk;if(r>=3)return{shouldHold:false,reason:"tp3_hit",newStop:s.direction==="LONG"?s.entry+risk*2:s.entry-risk*2,scaleOut:{level:s.tp3??s.entry,size:.2,label:"TP3_FINAL"}};if(r>=2)return{shouldHold:true,reason:"tp2_hit_lock_2r_runner",newStop:s.direction==="LONG"?s.entry+risk:s.entry-risk,scaleOut:{level:s.tp2??s.entry,size:.4,label:"TP2"}};if(r>=1)return{shouldHold:true,reason:"tp1_hit_scale_out_40",newStop:s.entry,scaleOut:{level:s.tp1??s.entry,size:.4,label:"TP1"}};}if((s.direction==="LONG"&&st.k>80)||(s.direction==="SHORT"&&st.k<20))return{shouldHold:true,reason:"stoch_extended_profit_protection"};const v=isSignalStillValid(s,p);return{shouldHold:v.valid,reason:v.reason};}
