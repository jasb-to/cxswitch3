...
export function generateSignal(pair:string,candles1h:Candle[],candles4h:Candle[],candles15m:Candle[],activeTrades?:any[],currentPrice?:number):SignalResult{
 ...
 // 4H 5/13 qualifies directional change. ENTRY_1 remains early by design.
 // The 4H 5/13 qualification for SHORT and LONG ENTRY_2 remains; no asset-specific filter is applied.
 if(raw!==null&&raw!=="ADD"){
   if(dir==="SHORT"&&ema513.direction!=="BEARISH"){debug.push(`FILTER: SHORT blocked — 4H regime is ${ema513.label}; short qualification requires bearish 4H 5/13`);raw=null;}
