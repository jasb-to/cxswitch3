...
function marketSnapshot(pair:string,c:Candle[],dir:"LONG"|"SHORT",tl:TrendlineResult,price:number,st?:{k:number;d:number},adxVal?:number){const closes=c.map(x=>x.close),near=tl.valid&&Math.abs((price-tl.price)/tl.price)<TL_THRESHOLD,beyond=tl.valid&&(dir==="LONG"?price>tl.price*1.008:price<tl.price*.992),e8=ema(closes,TF_FAST).at(-1)!,e21=ema(closes,TF_SLOW).at(-1)!,r=rsi(closes),stoch=st??stochRsi(closes);let momentumState="NEUTRAL";if((dir==="LONG"&&r>=80)||(dir==="SHORT"&&r<=20))momentumState="OVEREXTENDED";else if((dir==="LONG"&&r>=70)||(dir==="SHORT"&&r<=30))momentumState="HOT";else if((dir==="LONG"&&stoch.k<20)||(dir==="SHORT"&&stoch.k>80))momentumState="PULLBACK";return{pair,price:Math.round(price*100)/100,timestamp:Date.now(),trend:`${dir} ${strength(daily(c),dir)}`,location:near?"NEAR_TL":beyond?"BEYOND_TL":tl.valid?"FAR_FROM_TL":"TL_INVALIDATED",trigger:near?"READY":"WAITING",adx:adxVal??adx(c),rsi:Math.round(r*10)/10,stochK:stoch.k,stochD:stoch.d,trendlinePrice:tl.valid?Math.round(tl.price*100)/100:0,distToTrendline:tl.valid?Math.round(Math.abs((price-tl.price)/tl.price)*10000)/100:null,ema8_4h:Math.round(e8*100)/100,ema21_4h:Math.round(e21*100)/100,momentumState,trendlineStatus:tl.valid?"ACTIVE":tl.invalidated?"INVALIDATED":"REBUILDING",trendlineReason:tl.reason,trendlinePivots:tl.pivots.length,trendlineAgeCandles:tl.ageCandles,trendlineSlope:Math.round(tl.slope*100000)/100000};}
