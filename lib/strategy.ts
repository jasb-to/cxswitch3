// lib/strategy.ts — v28.5 "Trendline Retest ADD + Pivot Targets 1.2RR + ATRx2 Fallback + Stoch Extreme Block"
// ============================================================
// FIX: ADD signals now fire on retest of broken trendline, not when extended
...
export function generateSignal(
 ...
 const prev2 = candles4h[candles4h.length - 3];
 
 const closes4h = candles4h.map(c => c.close);
 const ema8_4h = ema(closes4h, 8);
 const ema21_4h = ema(closes4h, 21);
 
 // --- v28.5 ENTRY CONDITIONS ---
 
 // 1. NEAR TRENDLINE: Within 1.2% — accumulation zone
 const nearTrendline = Math.abs(dist) < 0.012;
 
 // 2. STOCH EXTREME: Oversold for LONG (K<20), Overbought for SHORT (K>80)
 const stochExtreme = t1d.direction === "LONG" ? stoch.k < 20 : stoch.k > 80;
 
 // 3. STOCH TURNING: K crossing with momentum shift
 const stochTurning = t1d.direction === "LONG" 
   ? stoch.k > stoch.d && prev.close <= prev2.close
   : stoch.k < stoch.d && prev.close >= prev2.close;
 
 // 4. RETEST ZONE: Price broke trendline and pulled back to it
 // LONG: price was above TL, now within 0.5%-2% above (retesting as support)
 // SHORT: price was below TL, now within 0.5%-2% below (retesting as resistance)
 ...
 // 5. CONFIRMING CANDLE: Directional momentum
 ...
 // Block ADD at Stoch extremes (no chasing tops/bottoms)
 ...
 // ENTRY_1: At trendline, Stoch extreme — accumulation at the low/high
 if (nearTrendline && stochExtreme) {
   rawType = "ENTRY_1";
 } 
 // ENTRY_2: At trendline, Stoch turning — early entry before extreme
 else if (nearTrendline && stochTurning && !stochExtreme && stoch.k > 10 && stoch.k < 90) {
   rawType = "ENTRY_2";
 } 
 // ADD: Retest of broken trendline with momentum — NOT when extended
 ...
 const signal: Signal = {
   ...
   reason: `${t1d.direction} ${type} ${finalType} | 1D ${t1d.strength} | Stoch K${stoch.k} D${stoch.d} | ${finalType === "ADD" ? "Retest+EMA" + (volUp ? "+Vol" : "") + (stochMomentum ? "+Stoch" : "") + (adxStrong ? "+ADX" : "") : "TL approach"} | TP:${targetSource} | RR ${rr.toFixed(2)}`,
   timestamp: now,
