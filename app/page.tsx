      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {PAIRS.map((pair) => {
            ...
            if (hasSignal) {
              ...
              if (mkt) {
                ...
                if (trendHealth === "STRONG") {
                  bannerText = `⏳ ${trendDir} STRONG — Waiting for entry`;
                  bannerClass = "bg-blue-600/50 text-blue-200";
                } else if (trendHealth === "MEDIUM") {
                  bannerText = `⏳ ${trendDir} MEDIUM — Watching`;
            ...
            const progress = hasSignal && status === "ACTIVE" && entry && target && currentPrice
              ...
              <div key={pair} className={`rounded-lg p-5 border-2 transition-all ${borderClass}`}>
                ...
                {mkt && (
                  <div className="mb-3 p-2 rounded border bg-gray-800/50 border-gray-600/50">
                    ...
                    {mkt.confirm1h !== undefined && (
                      <div className={`text-xs mt-1 font-bold ${mkt.confirm1h ? "text-green-400" : "text-red-400"}`}>
                        1H: {mkt.confirm1h ? "CONFIRMS ✓" : "REJECTS ✗"}
                      </div>
                    )}
                  </div>
                )}
                ...
                {!hasSignal && mkt && (
                  <div className="text-sm text-gray-500 space-y-1">
                    ...
                    <ul className="list-disc list-inside text-xs space-y-0.5">
                      ...
                      <li className={mkt.confirm1h ? "text-green-400" : "text-gray-500"}>
                        1H Confirmation: {mkt.confirm1h ? "Yes ✓" : "No ✗"}
                      </li>
                      <li className="text-gray-500">Entry TF: Waiting for sweep + cross</li>
                    </ul>
                  </div>
                )}
              </div>
