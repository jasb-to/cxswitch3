...
import { readSignals, writeSignals, getSignal, getTelegramCooldown, setTelegramCooldown } from "@/lib/persistent-store";
...
/**
 * EXECUTOR - Fetch Kraken prices, evaluate, store persistently
...
export async function GET() {
  try {
    ...
    // Read previous signals from persistent store
    ...
    return NextResponse.json({
      ok: true,
      message: "Signals evaluated and persisted",
      results,
    });
  } catch (error) {
