...
// The old 9-minute gate could suppress a legitimate scheduled run. Cron frequency is now separate from alert frequency.
// This is only a concurrency/manual-invocation guard; normal 10-minute scheduled runs are allowed.
...
function sameRecentSignal(history:any[],s:Signal,now:number){
  return history.some(h=>h.pair===s.pair&&h.direction===s.direction&&h.type===s.type&&now-h.timestamp<ADD_DEDUP_MS&&Math.abs((h.entry-s.entry)/s.entry)<ADD_DEDUP_ENTRY_PCT);
}
...
export async function GET(request:Request){
 ...
 if(started-last<MIN_CRON_INTERVAL_MS){
   console.log(`[CRON v${CXSWITCH_VERSION}] Guard: run skipped; previous run ${Math.round((started-last)/1000)}s ago`);
   return NextResponse.json({success:true,skipped:true,reason:"concurrency_guard"});
 }
 await setLastCronRun(started);
 console.log("========================================");
 console.log(`[CRON v${CXSWITCH_VERSION}] Started at ${new Date(started).toISOString()}`);
 ...
 for(const trade of [...active]){try{
  const c=await getCandles(krakenPairFormat(trade.pair+"/USD"),240);await sleep(API_DELAY_MS);
  const price=c.at(-1)?.close;if(price===undefined){console.log(`[MANAGE] ${trade.pair} — no price`);continue;}
  await updateActiveTradeMilestones(trade.id,price);await updateHistoryMilestones(trade.id,price);
  ...
  if(!hold.shouldHold){
    await updateSignalHistoryStatus(trade.id,hold.reason==="tp3_hit"||hold.reason==="tp2_hit_lock_2r"?"TP_HIT":"FAILED",hold.reason,price);
    active=active.filter(x=>x.id!==trade.id);alerts.push({pair:trade.pair,status:"exit",reason:hold.reason,price});
    console.log(`[EXIT] ${trade.pair} ${trade.direction} — ${hold.reason} @ ${price}`);continue;
  }
 ...
 for(const pair of PAIRS){try{
  ...
  if(signal.type==="ADD"&&!hasSameDirection){console.log(`[PAIR] ${pair} — ADD blocked: no active same-direction position`);continue;}
  ...
  console.log(`[ALERT] ${pair} — ${signal.type} sent @ ${signal.entry} | SL ${signal.stop} | TP1 ${signal.tp1} | TP2 ${signal.tp2} | TP3 ${signal.tp3}`);
