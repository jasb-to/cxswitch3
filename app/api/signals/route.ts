...
function managementAdvice(h:any,m:any){
  ...
  const entry513Direction=h.context?.fourH513?.direction||h.fourH513Direction||h.fourH513?.direction;
  const fourHWasAlreadyAgainstAtEntry=entry513Direction!==undefined && entry513Direction!==(long?"BULLISH":"BEARISH");
...
export async function GET(){
  ...
  // An active alert owns the card's state. This prevents the board from immediately
  // falling back to the pre-alert WAITING snapshot when ENTRY_1 fires.
  const liveMarketData=(Array.isArray(marketData)?marketData:[]).map((m:any)=>{
    const a:any=latestAlerts[m.pair];
    if(!a||a.status!=="ACTIVE")return m;
    return {...m,price:a.currentPrice??m.price,location:a.context?.marketPhase||m.location,trigger:a.trigger||m.trigger,alertState:a.validity?.state||"VALID",alertType:a.type,alertDirection:a.direction,alertEntry:a.entry,alertTimestamp:a.timestamp};
  });
  ...
  const logs=[
    `[SYSTEM] CXSwitch v${CXSWITCH_VERSION} | ${ENTRY_ARCHITECTURE} | ${DAILY_BIAS} | ${EXECUTION_MODE}`,
    ...
    `[CRON] State: active=${enrichedActive.length} marketData=${liveMarketData.length} history=${signalHistory.length} latest=${Object.keys(latestAlerts).length}`
  ].slice(0,40);
