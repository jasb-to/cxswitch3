...
function managementAdvice(h:any,m:any){
  ...
  const e=m.fourH513;if(!e)return{managementState:"STAY",status:"healthy",recommendation:"HOLD — SL UNCHANGED",reason:"Trade remains active. Do not raise the stop before R1; the next management point is R1, then move SL to breakeven."};
  ...
  if(tp2Hit){if(same513&&same821&&!contracting&&!exhausted&&momentum!=="OVEREXTENDED")return{managementState:"STAY",status:"healthy",recommendation:"R2 RUNNER POSSIBLE",reason:"R1.5 reached. 4H 5/13 remains aligned and 8/21 confirms the move — keep the R2 runner unless momentum deteriorates."};return{managementState:"PROTECT",status:"warning",recommendation:"R1.5 IS THE LIKELY FINAL TARGET",reason:"R1.5 reached. The remaining position should be protected; do not assume R2."};}
  if(tp1Hit){if(against513&&!fourHWasAlreadyAgainstAtEntry)return{managementState:"DEFEND",status:"failed",recommendation:"PROTECT PROFIT",reason:"R1 reached and 4H 5/13 has turned against the position. Move SL to breakeven if not already done and protect the remaining profit."};if(contracting||exhausted||weakMomentum||!same821)return{managementState:"PROTECT",status:"warning",recommendation:"SL TO BREAKEVEN · R1.5 LIKELY FINAL",reason:`R1 reached. Move SL to breakeven. Momentum is weakening${contracting?" (5/13 spread contracting)":""}${weakMomentum?` (${momentum})`:""}${exhausted?" (Stoch exhaustion)":""}${!same821?" (8/21 not aligned)":""}. Do not assume R2.`};return{managementState:"STAY",status:"healthy",recommendation:"SL TO BREAKEVEN · HOLD FOR R1.5",reason:"R1 reached. Move SL to breakeven. 4H 5/13 and 8/21 remain aligned, so R1.5 remains the next management target."};}
  if(against513&&!fourHWasAlreadyAgainstAtEntry)return{managementState:"DEFEND",status:"failed",recommendation:"TRADE VALIDITY WARNING · PROTECT",reason:"4H 5/13 has turned against the position after entry. The alert may still be above/below its original SL, but the move is deteriorating. Do not widen the SL; reassess manually."};
  if(against513&&fourHWasAlreadyAgainstAtEntry)return{managementState:"DEFEND",status:"warning",recommendation:"HOLD — 4H RECOVERY NEEDED",reason:`4H 5/13 is still ${long?"bearish":"bullish"}, as it was at entry. This is an early V28 position; do not widen the SL. Watch for ${long?"bullish":"bearish"} 4H recovery.`};
  if(contracting||exhausted||weakMomentum||!same821)return{managementState:"PROTECT",status:"warning",recommendation:"HOLD — SL UNCHANGED · WATCH R1", reason:`Trade remains active. ${contracting?"Wave cooling":weakMomentum?"Normal pullback":exhausted?"Move is extended":!same821?"8/21 is no longer fully aligned":"Wave is active"} — no confirmed reversal. Next management point: R1 → SL to breakeven.`};
  return{managementState:"STAY",status:"healthy",recommendation:"HOLD — SL UNCHANGED · TARGET R1",reason:"Wave is active and the trade remains valid. Keep the original SL. At R1, move SL to breakeven; R1.5 is next, then R2 runner if the wave remains active."};
}
